...
unsigned int VPN_to_PFN_address_translation(unsigned long int vpn);
...
unsigned int VPN_to_PFN_address_translation(unsigned long int vpn)
{
    ...
    fprintf(output_file, "Translating. Translation for VPN %ld not found in page table\n", vpnBits);
    exit(1);
